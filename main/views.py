...
def invoice(request):
	...
	textOb.setWordSpace(50)
	textOb.setFont("Helvetica",16)

	
	lines = request.user.userprofile.skills.filter(is_key_skill=True)
	sk = Certificate.objects.values_list('id','name')

	textOb.textLine("Order Invoice Mediseve pvt ltd ")
	textOb.textLine(" ")
	textOb.textLine(" ")
	textOb.textLine("(id, product, price/unit, total)")
	#for line in lines:
		#textOb.textLine(str(line))
	for skil in sk:
		textOb.textLine(str(skil))
	textOb.textLine("-----------------------------------------------------------------------")
	textOb.textLine("Grand Total-------------------------------------1999")
